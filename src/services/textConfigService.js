// Service for managing EvoluSoft homepage text configuration
class TextConfigService {
  constructor() {
    this.storageKey = 'evolusoftTexts';
    this.defaultTexts = {
      // Hero Section
      ...
      // Contact Information
      headquartersAddress: '16, BT4-3, Vinaconex 3 - Trung Van, Nam Tu Liem, Hanoi, Vietnam',
      hotlineNumber: '(024) 73046618',
      supportEmail: 'support@evolusoft.vn',
      workingHoursAM: '8:00 - 17:00',
      workingHoursPM: '8:00 - 12:00',
  ...
  getText(key) {
    ...
    return texts[key] || this.defaultTexts[key] || '';
  }
  ...
  saveTexts(texts) {
    try {
      ...
      // Also save to a backup key that can be accessed cross-origin
      localStorage.setItem('evolusoftTexts_backup', JSON.stringify(texts));
      
      // Try to also save to sessionStorage as a backup
      sessionStorage.setItem(this.storageKey, JSON.stringify(texts));
      
      // Trigger a custom event for cross-tab communication
      window.dispatchEvent(new CustomEvent('evolusoftTextsUpdated', { 
        detail: texts 
      }));
      
      console.log('✅ Texts saved successfully:', texts);
      return true;
    } catch (error) {
      console.error('Error saving text configuration:', error);
      return false;
    }
  }
  ...
  resetTexts() {
    try {
      ...
      console.error('Error resetting text configuration:', error);
      return false;
    }
  }
